...
RadCooling::RadCooling(Input &input, Grid &grid, Fluid<Phys> *hydroin):
  ...
  /* TRML extras */
  if (input.CheckEntry("Setup", "Tcl") <= 0) {
    IDEFIX_ERROR("Setup:Tcl is not defined in the input file. "
                 "This is required for the TRML setup.");
  }
  if (input.CheckEntry("Setup", "chi") <= 0) {
    IDEFIX_ERROR("Setup:chi is not defined in the input file. "
                 "This is required for the TRML setup.");
  }

  this->Tcl     = input.Get<real>("Setup", "Tcl", 0);
  this->chi     = input.Get<real>("Setup", "chi", 0);
