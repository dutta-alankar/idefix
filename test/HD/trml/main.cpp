...
// Defined in setup.cpp: frees setup-owned globals that hold Kokkos Views (the
// cooling table). Must be called before Kokkos::finalize().
...
int main( int argc, char* argv[] ) {
  ...
  // Release setup-owned globals holding Kokkos Views before finalize, otherwise
  // their static-lifetime destructors run after Kokkos::finalize() and Kokkos
  // aborts ("... is being deallocated after Kokkos::finalize was called").
  CleanupGlobalInSetup();
