...
#endif
{
  ...
  if(input.CheckEntry("Output","uservar")>0) {
    ...
    for(int var = 0 ; var < nvars ; var++) {
      ...
      #ifdef WITH_HDF5
      // Also register user-defined variables with the XDMF/HDF5 writer, otherwise
      // they are written to VTK only and are missing from the HDF5 (xdmf) dumps.
      data.xdmf->RegisterVariable(userDefVariables[arrayName],arrayName);
      #endif
    }
