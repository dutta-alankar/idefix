...
void RadCooling::InitArrays() {
  ...
  delta_eng = IdefixArray3D<real>("delta_eng_cooling_source", data->np_tot[KDIR],
                                                              data->np_tot[JDIR],
                                                              data->np_tot[IDIR]);
  delta_eng_total = IdefixArray3D<real>("delta_eng_cooling_source_total", data->np_tot[KDIR],
                                                              data->np_tot[JDIR],
                                                              data->np_tot[IDIR]);
  auto delta_eng = this->delta_eng;
  auto delta_eng_total = this->delta_eng_total;
  delta_eng_total_host = Kokkos::create_mirror_view(Kokkos::HostSpace(), delta_eng_total);
  ...
  idefix_for("RadCoolingLoopInitialize",kbeg, kend, jbeg, jend, ibeg, iend,
    KOKKOS_LAMBDA (int k, int j, int i) {
      delta_eng(k,j,i) = ZERO_F;
      delta_eng_total(k,j,i) = ZERO_F;
    });
}
...
void RadCooling::TownsendIntegration(real dt) {
  ...
  /* TRML extras */
  [[maybe_unused]] const real Tcl = this->Tcl;
  [[maybe_unused]] const real chi = this->chi;
  ...
  idefix_for("RadCoolingLoop",kbeg, kend, jbeg, jend, ibeg, iend,
    KOKKOS_LAMBDA (int k, int j, int i) {
      ...
      if ((temperature < temperature_min_data) ||
        ...
        // Temperature outside the tabulated range: apply no cooling for this
        // cell and continue (matches the reference; do not abort the run).
        delta_eng(k,j,i) = ZERO_F;
        return;
      }

      if (temperature<=TcoolFloor) {
        // Below the floor: enforce the temperature floor rather than disabling
        ...
        if ( delta_eng(k,j,i)<ZERO_F ) {
          delta_eng_total(k,j,i) += delta_eng(k,j,i);
        }
      }
    }
  );
